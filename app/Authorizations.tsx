...
export default function Authorizations({locations,currentUser,apiFetch,setNotice}:Props){
  ...
  async function createRequest(form:FormData){
    if(saving)return;
    const needsEvidence=type==="incentive";
    const evidence=needsEvidence?form.get("evidence") as File|null:null;
    if(needsEvidence&&!evidence?.size){setNotice("Adjunta una fotografía de constancia");return;}
    if(evidence&&evidence.size>8*1024*1024){setNotice("La constancia no puede superar 8 MB");return;}
    if(evidence&&!evidence.type.startsWith("image/")){setNotice("La constancia debe ser una imagen");return;}
    setSaving(true);
    let evidencePath="";
    try{
      if(evidence){
      ...
      const response=await apiFetch("/api/authorizations",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({
        action:"create",requestType:type,locationId:Number(form.get("locationId")),shopperName:form.get("shopperName"),shopperExternalId:form.get("shopperExternalId"),orderNumber:form.get("orderNumber"),clientName:form.get("clientName"),amount:form.get("amount"),reason:form.get("reason"),evidencePath:evidencePath||null
      })});
