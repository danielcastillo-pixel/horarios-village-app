...
type RequestRow={
  id:number;request_type:"discount"|"incentive";location_id:number;location_name_snapshot:string;
  shopper_name:string;shopper_external_id:string;order_number:string;client_name:string;amount:number;reason:string;
  evidence_path:string|null;evidence_paths:string[];evidence_url:string|null;evidence_urls:string[];needs_help:boolean;
  status:"pending"|"approved"|"rejected";decision_comment:string;created_by_name:string;created_by_email:string;
  created_at:string;decided_by_name:string|null;decided_at:string|null
};
...
export default function Authorizations({locations,currentUser,apiFetch,setNotice}:Props){
  ...
  const [year,setYear]=useState(Math.max(2026,now.getFullYear())),[month,setMonth]=useState(now.getMonth()+1);
  ...
  async function createRequest(form:FormData){
    if(saving)return;
    const evidenceFiles=(form.getAll("evidence") as FormDataEntryValue[]).filter((entry):entry is File=>entry instanceof File&&entry.size>0);
    if(type==="incentive"&&!evidenceFiles.length){setNotice("Adjunta al menos una fotografía de constancia");return;}
    if(evidenceFiles.length>10){setNotice("Puedes subir un máximo de 10 fotografías");return;}
    const invalid=evidenceFiles.find(file=>file.size>8*1024*1024||!file.type.startsWith("image/"));
    if(invalid){setNotice(invalid.size>8*1024*1024?"Cada constancia puede pesar máximo 8 MB":"Todos los archivos deben ser imágenes");return;}
    setSaving(true);
    ...
    try{
      if(evidenceFiles.length){
        ...
        for(const evidence of evidenceFiles){
          ...
          const {error:uploadError}=await supabase.storage.from("authorization-evidence").upload(path,evidence,{contentType:evidence.type,upsert:false});
          if(uploadError)throw new Error(`No se pudo subir ${evidence.name}: ${uploadError.message}`);
      ...
      const response=await apiFetch("/api/authorizations",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({
        action:"create",requestType:type,locationId:Number(form.get("locationId")),shopperName:form.get("shopperName"),
        shopperExternalId:form.get("shopperExternalId"),orderNumber:form.get("orderNumber"),clientName:form.get("clientName"),
        amount:form.get("amount"),reason:form.get("reason"),evidencePaths,needsHelp:form.get("needsHelp")==="on"
      })});
  ...
  const monthlySummary=useMemo(()=>locations.map(location=>({
    location,
    discounts:requests.filter(row=>row.location_id===location.id&&row.status==="approved"&&row.request_type==="discount").length,
    incentives:requests.filter(row=>row.location_id===location.id&&row.status==="approved"&&row.request_type==="incentive").length
  })).filter(row=>row.discounts||row.incentives),[locations,requests]);

  return <section className="authorization-module">
    <div className="authorization-hero"><div><span>AUTORIZACIONES OPERATIVAS</span><h2>Descuentos e incentivos</h2><p>{isAdmin?"Revisa constancias, solicitudes de ayuda y decide cada caso.":"Envía solicitudes, adjunta constancias y pide ayuda cuando la necesites."}</p></div><span className="authorization-lock">{isAdmin?"Control privado del administrador":"Histórico protegido por local"}</span></div>
    ...
    {tab==="new"&&!isAdmin?<form className="authorization-form" onSubmit={event=>{event.preventDefault();void createRequest(new FormData(event.currentTarget));}}>
      ...
      <div className="authorization-grid">
        <label>Local<select name="locationId" required>{locations.map(location=><option key={location.id} value={location.id}>{location.name}</option>)}</select></label>
        <label>Supervisor<input value={currentUser.name} disabled /></label>
        <label>Nombre del shopper<input name="shopperName" required /></label>
        <label>ID del shopper<input name="shopperExternalId" placeholder="Opcional" /></label>
        <label>Número de pedido<input name="orderNumber" required /></label>
        <label>Nombre del cliente<input name="clientName" required /></label>
        <label>Valor solicitado<input name="amount" type="number" min="0.01" step="0.01" required /></label>
        <label className="wide">{type==="discount"?"Razón del descuento":"Razón del incentivo"}<textarea name="reason" rows={4} required minLength={5}/></label>
        {type==="incentive"&&<label className="wide evidence-upload">Constancias fotográficas<input name="evidence" type="file" multiple accept="image/jpeg,image/png,image/webp,image/heic,image/heif" required/><small>Puedes seleccionar varias fotos · máximo 8 MB por fotografía</small></label>}
        <label className="wide authorization-help-check"><input name="needsHelp" type="checkbox"/><span>{type==="discount"?"¿Necesitas ayuda para subir este descuento?":"¿Necesitas ayuda para subir este incentivo?"}</span></label>
      </div>
      ...
      {loading?<div className="authorization-empty">Cargando solicitudes…</div>:visible.length?<div className="authorization-cards">{visible.map(row=>{
        const urls=row.evidence_urls?.length?row.evidence_urls:row.evidence_url?[row.evidence_url]:[];
        return <article key={row.id} className={`authorization-request ${row.status}`}>
          <div className="request-main"><span className={`request-type ${row.request_type}`}>{row.request_type==="discount"?"Descuento":"Incentivo"}</span>{row.needs_help&&<span className="request-help-badge">Necesita ayuda</span>}<h3>{row.shopper_name}</h3><p>{row.location_name_snapshot} · Pedido #{row.order_number}</p><small>Cliente: {row.client_name}</small></div>
          <div className="request-amount"><strong>{money(Number(row.amount))}</strong><span className={`request-status ${row.status}`}>{statusLabel(row.status)}</span></div>
          <div className="request-detail"><p><strong>Motivo</strong>{row.reason}</p><p><strong>Enviado por</strong>{row.created_by_name}<small>{timeLabel(row.created_at)}</small></p>{row.needs_help&&<p className="request-help-message"><strong>Ayuda solicitada</strong>El supervisor marcó que necesita apoyo para subir esta solicitud.</p>}{row.status!=="pending"&&<p><strong>Decisión</strong>{row.decided_by_name||"Administrador"}<small>{row.decision_comment||"Sin comentario"}</small></p>}</div>
          <div className="request-actions">{urls.length?<div className="request-evidence-gallery">{urls.map((url,index)=><a key={url} href={url} target="_blank" rel="noreferrer">Ver constancia {index+1}</a>)}</div>:<span>{row.request_type==="discount"?"Sin constancia requerida":"Constancia no disponible"}</span>}{isAdmin&&row.status==="pending"&&<><button className="approve" onClick={()=>setDecision({request:row,status:"approved"})}>Autorizar</button><button className="reject" onClick={()=>setDecision({request:row,status:"rejected"})}>Rechazar</button></>}</div>
        </article>;
      })}</div>:<div className="authorization-empty">No existen solicitudes para los filtros seleccionados.</div>}
    </div>}
