...
const activityLabels={shopper_purchase:"Horario shoppers · Compra",shopper_delivery:"Horario shoppers · Entrega",administrator_rating:"Calificación administrador",supervisor_schedule:"Horario supervisor"} as const;
...
export default function WeeklyCompliance({locations,currentUser,apiFetch,setNotice}:Props){
  ...
  const rows=useMemo(()=>locations.map(location=>{
    ...
    return {location,entries,completed,percentage:completed*25};
  }),[locations,submissions,evaluations,weekStart]);
  const totals=useMemo(()=>({completed:rows.reduce((sum,row)=>sum+row.completed,0),required:rows.length*4,full:rows.filter(row=>row.percentage===100).length,pending:rows.filter(row=>row.percentage<100).length}),[rows]);
  ...
  return <section className="compliance-module">
    <div className="compliance-hero"><div><span>CONTROL AUTOMÁTICO</span><h2>Cumplimiento semanal</h2><p>Compra, entrega, calificación del administrador y horario de supervisión, con autor y fecha reales.</p></div><span className="admin-lock">Administrador: solo tú</span></div>
    ...
    <div className="compliance-kpis"><article><span>Cumplimiento regional</span><strong>{overall}%</strong><div className="compliance-progress"><i style={{width:`${overall}%`}}/></div></article><article><span>Actividades completas</span><strong>{totals.completed}/{totals.required}</strong><small>4 obligatorias por local</small></article><article><span>Locales al 100%</span><strong>{totals.full}</strong><small>Semana seleccionada</small></article><article><span>Locales pendientes</span><strong>{totals.pending}</strong><small>Requieren seguimiento</small></article></div>
    <div className="compliance-layout"><div className="compliance-table-card"><div className="compliance-table-title"><div><h3>{dateLabel(weekStart)} — {dateLabel(moveDate(weekStart,6))}</h3><p>{loading?"Cargando actividad…":"Cada actividad completa equivale al 25%."}</p></div></div><div className="table-wrap"><table className="compliance-table"><thead><tr><th>Local</th>{(Object.keys(activityLabels) as Activity[]).map(activity=><th key={activity}>{activityLabels[activity]}</th>)}<th>Total</th></tr></thead><tbody>{rows.map(row=><tr key={row.location.id}><td><strong>{row.location.name}</strong><small>{row.location.city}</small></td>{(Object.keys(activityLabels) as Activity[]).map(activity=>{const entry=row.entries[activity];return <td key={activity}>{entry?<div className="compliance-done"><b>✓ 100%</b><strong>{entry.submitted_by_name}</strong><small>{timeLabel(entry.submitted_at)}</small>{entry.last_updated_by_name!==entry.submitted_by_name&&<small>Actualizó: {entry.last_updated_by_name}</small>}{entry.submitted_by_email.toLowerCase()===currentUser.email.toLowerCase()&&<em>Hecho por administrador</em>}</div>:<div className="compliance-missing"><b>0%</b><span>Pendiente</span></div>}</td>})}<td><span className={`compliance-total ${row.percentage===100?"full":row.percentage?"partial":"empty"}`}>{row.percentage}%</span></td></tr>)}</tbody></table></div></div><aside className="compliance-ranking"><h3>Actividad por supervisor</h3><p>Quién registró las funciones de esta semana.</p>{actors.length?actors.map((actor,index)=><article key={actor.email}><span>{index+1}</span><div><strong>{actor.name}</strong><small>{actor.admin?"Administrador":"Supervisor"}</small></div><b>{actor.count}/4</b></article>):<div className="compliance-ranking-empty">Todavía no existen actividades publicadas.</div>}<small className="ranking-note">Las actividades hechas por el administrador resuelven la operación, pero quedan identificadas y no se atribuyen a un supervisor.</small></aside></div>
  </section>;
}
