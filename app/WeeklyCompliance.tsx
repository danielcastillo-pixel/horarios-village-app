...
const activityLabels={shopper_purchase:"Horario shoppers · Compra",shopper_delivery:"Horario shoppers · Entrega",administrator_rating:"Calificación administrador",supervisor_schedule:"Horario supervisor",automatic_assignment:"Constancia · Asignación automática",team_meeting:"Constancia · Reunión semanal",supervisor_meeting:"Constancia · Reunión de supervisores"} as const;
...
export default function WeeklyCompliance({locations,currentUser,apiFetch,setNotice}:Props){
  ...
  const totals=useMemo(()=>({completed:rows.reduce((sum,row)=>sum+row.completed,0),required:rows.length*requiredActivityCount,full:rows.filter(row=>row.percentage===100).length,pending:rows.filter(row=>row.percentage<100).length}),[rows]);
