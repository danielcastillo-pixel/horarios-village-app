...
type AiKpi={city:string;sales:number|null;orders:number|null;averageTicket:number|null;latePercent:number|null;reschedulingPercent:number|null;oosPercent:number|null;confidence:number;warnings:string[]};
...
type Modal="kpi"|"meeting"|"request"|"import"|"ai"|null;
...
export default function RegionalProfitability({locations,currentUser,apiFetch,setNotice}:Props){
  ...
  const [aiFile,setAiFile]=useState<File|null>(null),[aiRows,setAiRows]=useState<AiKpi[]>([]),[aiWarning,setAiWarning]=useState(""),[analyzing,setAnalyzing]=useState(false);
  ...
  function openAi(){setAiFile(null);setAiRows([]);setAiWarning("");setModal("ai");}
  ...
  async function analyzeCapture(){
    ...
    if(aiFile.size>8*1024*1024){setNotice("La captura puede pesar máximo 8 MB");return;}
    setAnalyzing(true);setAiRows([]);setAiWarning("");
    try{
      const form=new FormData();form.append("image",aiFile);
      const response=await apiFetch("/api/regional-performance/ai",{method:"POST",body:form}),payload=await response.json().catch(()=>({error:"No se pudo analizar la captura"})) as {rows?:AiKpi[];imageWarning?:string;error?:string};
      if(!response.ok)throw new Error(payload.error||"No se pudo analizar la captura");
      setAiRows(payload.rows||[]);setAiWarning(payload.imageWarning||"");setNotice(`✓ ${(payload.rows||[]).length} ciudades leídas. Revisa antes de guardar.`);
    }catch(error){setNotice(`Error: ${error instanceof Error?error.message:"No se pudo analizar la captura"}`);}finally{setAnalyzing(false);}
  }

  async function saveAiRows(){
    if(!aiRows.length||saving)return;setSaving(true);
    try{const result=await post({action:"importKpis",weekStart,rows:aiRows});setNotice(`✓ ${result.imported||aiRows.length} ciudades actualizadas desde la captura`);setModal(null);setAiRows([]);await loadData();}
    catch(error){setNotice(`Error: ${error instanceof Error?error.message:"No se pudieron guardar los KPI"}`);}finally{setSaving(false);}
  }
  ...
  return <section className="regional-module">
    ...
    <div className="regional-week-toolbar"><button onClick={()=>setWeekStart(moveDate(weekStart,-7))}>‹</button><label>Semana<input type="date" value={weekStart} onChange={event=>setWeekStart(mondayFor(event.target.value))}/></label><strong>{dateLabel(weekStart)} — {dateLabel(moveDate(weekStart,6))}</strong><button onClick={()=>setWeekStart(moveDate(weekStart,7))}>›</button><button className="regional-upload regional-ai-button" onClick={openAi}>✦ Cargar captura con IA</button><button className="regional-upload" onClick={()=>{setImportRows([]);setImportFileName("");setModal("import")}}>▧ Cargar KPIs por Excel</button></div>
    ...
    {modal==="ai"&&<div className="modal-backdrop"><div className="modal regional-form regional-import regional-ai-modal"><button type="button" className="close" onClick={()=>setModal(null)}>×</button><span className="regional-ai-eyebrow">ASISTENTE DE KPI</span><h2>Cargar captura con IA</h2><p>Semana del {dateLabel(weekStart)}. La IA leerá solo los datos acordados y no guardará nada hasta que confirmes.</p><div className="regional-ai-scope"><strong>Solo se actualizarán</strong><span>Ciudad</span><span>Pedidos</span><span>GMV</span><span>AOV</span><span>Tarde %</span><span>Reagendados %</span><span>Pedidos OOS %</span><small>Clientes activos, margen, incidencias y textos existentes no se modificarán.</small></div><div className="regional-import-actions"><label>Seleccionar captura<input type="file" accept="image/jpeg,image/png,image/webp" onChange={event=>{setAiFile(event.target.files?.[0]||null);setAiRows([]);setAiWarning("")}}/><small>JPG, PNG o WebP · máximo 8 MB</small></label><button className="secondary regional-ai-analyze" type="button" disabled={!aiFile||analyzing} onClick={()=>void analyzeCapture()}>{analyzing?"Analizando captura…":"✦ Analizar captura"}</button></div>{aiFile&&<strong className="regional-file-name">{aiFile.name}</strong>}{aiWarning&&<div className="regional-ai-warning">{aiWarning}</div>}{aiRows.length>0&&<><div className="table-wrap regional-preview regional-ai-preview"><table><thead><tr><th>Ciudad</th><th>Pedidos</th><th>GMV</th><th>AOV</th><th>Tarde</th><th>Reag.</th><th>Pedidos OOS</th><th>Lectura</th></tr></thead><tbody>{aiRows.map((row,index)=><tr key={`${row.city}-${index}`}><td><strong>{row.city}</strong>{row.warnings.length>0&&<small title={row.warnings.join(" · ")}>Revisar dato</small>}</td><td>{row.orders??"No leído"}</td><td>{row.sales===null?"No leído":money(row.sales)}</td><td>{row.averageTicket===null?"No leído":money(row.averageTicket)}</td><td>{row.latePercent===null?"No leído":percent(row.latePercent)}</td><td>{row.reschedulingPercent===null?"No leído":percent(row.reschedulingPercent)}</td><td>{row.oosPercent===null?"No leído":percent(row.oosPercent)}</td><td><span className={`regional-ai-confidence ${row.confidence<.8?"review":"ok"}`}>{row.confidence<.8?"Revisar":"Clara"}</span></td></tr>)}</tbody></table></div><div className="regional-ai-confirm"><span>Revisa la tabla. “No leído” significa que ese valor conservará el dato que ya tenía.</span><button className="primary" type="button" disabled={saving} onClick={()=>void saveAiRows()}>{saving?"Guardando…":`Guardar ${aiRows.length} ciudades`}</button></div></>}</div></div>}
  </section>;
}
