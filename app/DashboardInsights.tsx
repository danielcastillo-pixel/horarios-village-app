...
type Evidence={location_id:number;week_start:string;evidence_type:"automatic_assignment"|"team_meeting"|"supervisor_meeting";submitted_by_name:string;submitted_at:string};
...
export default function DashboardInsights({locations,apiFetch,setNotice,onNavigate}:Props){
  ...
  const [submissions,setSubmissions]=useState<Submission[]>([]),[evaluations,setEvaluations]=useState<Evaluation[]>([]),[evidences,setEvidences]=useState<Evidence[]>([]),[requests,setRequests]=useState<Authorization[]>([]),[loading,setLoading]=useState(true);
  ...
  useEffect(()=>{(async()=>{
    ...
    try{
      ...
      const compliance=await complianceResponse.json().catch(()=>({error:"No disponible"})) as {submissions?:Submission[];evaluations?:Evaluation[];evidences?:Evidence[];error?:string};
      ...
      setSubmissions(compliance.submissions||[]);setEvaluations(compliance.evaluations||[]);setEvidences(compliance.evidences||[]);setRequests(authorizations.requests||[]);
  ...
  const localCompliance=useMemo(()=>locations.map(location=>{
    ...
    evidences.filter(row=>row.location_id===location.id&&row.week_start===weekStart).forEach(row=>activity.add(row.evidence_type));
    const completed=activity.size;
    return {...location,completed,pending:requiredActivityCount-completed,percentage:Math.round(completed/requiredActivityCount*100)};
  ...
  return <section className="dashboard-insights">
    ...
    <div className="dashboard-alert-grid">
      ...
      <button className={`dashboard-alert tasks ${incomplete?"has-alert":""}`} onClick={()=>onNavigate("Cumplimiento semanal")}><i>✓</i><div><span>Actividades pendientes</span><strong>{loading?"—":incomplete}</strong><small>De {locations.length*requiredActivityCount} obligaciones regionales</small></div><b>Ver cumplimiento →</b></button>
    </div>
