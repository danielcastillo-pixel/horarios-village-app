...
type Evidence={location_id:number;week_start:string;evidence_type:"automatic_assignment"|"team_meeting";submitted_by_name:string;submitted_at:string};
...
export default function DashboardInsights({locations,apiFetch,setNotice,onNavigate}:Props){
  ...
  const localCompliance=useMemo(()=>locations.map(location=>{
    ...
    return {...location,completed,pending:6-completed,percentage:Math.round(completed/6*100)};
  ...
  return <section className="dashboard-insights">
    ...
    <div className="dashboard-alert-grid">
      ...
      <button className={`dashboard-alert tasks ${incomplete?"has-alert":""}`} onClick={()=>onNavigate("Cumplimiento semanal")}><i>✓</i><div><span>Actividades pendientes</span><strong>{loading?"—":incomplete}</strong><small>De {locations.length*6} obligaciones regionales</small></div><b>Ver cumplimiento →</b></button>
    </div>
