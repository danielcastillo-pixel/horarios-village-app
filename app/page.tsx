...
type FillDrag={kind:FillKind;sourceIndex:number;targetIndex:number;sourceDay:number;targetDay:number;pointerId:number};
...
export default function Home() {
  ...
  const [showShopperHours,setShowShopperHours]=useState(false);
  ...
  const [editShopperShift,setEditShopperShift]=useState<ShopperShiftType|null>(null);
  const [deleteShopperShift,setDeleteShopperShift]=useState<ShopperShiftType|null>(null);
  const [deletingShopperShift,setDeletingShopperShift]=useState(false);
  ...
  },[accessState,active,shopperView,shopperCategory,weekStart,location,data?.currentUser.email]);
  useEffect(() => {
    if (!data) return;
  ...
  function beginFillDrag(kind:FillKind,sourceIndex:number,day:number,event:ReactPointerEvent<HTMLButtonElement>){
    ...
    setActiveFillDrag({kind,sourceIndex,targetIndex:sourceIndex,sourceDay:day,targetDay:day,pointerId:event.pointerId});
  }
  ...
  function moveFillDrag(event:ReactPointerEvent<HTMLButtonElement>){
    ...
    const tableWrap=event.currentTarget.closest<HTMLElement>(".table-wrap");
    if(event.clientX>window.innerWidth-70)tableWrap?.scrollBy({left:28,behavior:"auto"});
    else if(event.clientX<70)tableWrap?.scrollBy({left:-28,behavior:"auto"});
    const element=document.elementFromPoint(event.clientX,event.clientY);
    const cell=element?.closest<HTMLElement>(`td[data-fill-kind="${current.kind}"]`);
    const targetIndex=Number(cell?.dataset.fillRow);
    const targetDay=Number(cell?.dataset.fillDay);
    if(!Number.isInteger(targetIndex)||!Number.isInteger(targetDay)||targetDay<0||targetDay>6||(targetIndex===current.targetIndex&&targetDay===current.targetDay))return;
    setActiveFillDrag({...current,targetIndex,targetDay});
  }

  function fillCellClass(kind:FillKind,row:number,day:number){
    if(!fillDrag||fillDrag.kind!==kind)return "";
    if(row===fillDrag.sourceIndex&&day===fillDrag.sourceDay)return " fill-source";
    const rowStart=Math.min(fillDrag.sourceIndex,fillDrag.targetIndex),rowEnd=Math.max(fillDrag.sourceIndex,fillDrag.targetIndex);
    const dayStart=Math.min(fillDrag.sourceDay,fillDrag.targetDay),dayEnd=Math.max(fillDrag.sourceDay,fillDrag.targetDay);
    return row>=rowStart&&row<=rowEnd&&day>=dayStart&&day<=dayEnd?" fill-target":"";
  }

  function fillTargets(drag:FillDrag){
    const targets:{row:number;day:number}[]=[];
    const rowStart=Math.min(drag.sourceIndex,drag.targetIndex),rowEnd=Math.max(drag.sourceIndex,drag.targetIndex);
    const dayStart=Math.min(drag.sourceDay,drag.targetDay),dayEnd=Math.max(drag.sourceDay,drag.targetDay);
    for(let row=rowStart;row<=rowEnd;row++)for(let day=dayStart;day<=dayEnd;day++)if(row!==drag.sourceIndex||day!==drag.sourceDay)targets.push({row,day});
    return targets;
  }

  async function fillSupervisorCells(drag:FillDrag){
    const source=filtered[drag.sourceIndex];
    const sourceShift=source?.shifts[drag.sourceDay];
    const targetCells=fillTargets(drag).map(target=>({person:filtered[target.row],day:target.day})).filter(target=>Boolean(target.person));
    if(!source||!sourceShift||!targetCells.length)return;
    const overwriteCount=targetCells.filter(target=>Boolean(target.person.shifts[target.day])).length;
    ...
    const targetDays=new Map<number,Set<number>>();
    targetCells.forEach(target=>{const days=targetDays.get(target.person.id)||new Set<number>();days.add(target.day);targetDays.set(target.person.id,days);});
    setPeople(current=>current.map(person=>targetDays.has(person.id)
      ?{...person,shifts:person.shifts.map((item,index)=>targetDays.get(person.id)?.has(index)?copiedShift:item)}
      :person));
    setNotice(`Copiando turno en ${targetCells.length} celda${targetCells.length===1?"":"s"}…`);
    const response=await apiFetch("/api/data",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({
      action:"fillAssignments",
      assignments:targetCells.map(target=>({supervisorId:target.person.id,workDate:dateKeys[target.day],start:free?null:times[0],end:free?null:times[1],roleId:roleRow.id}))
    })});
    ...
    setNotice(`✓ Turno copiado en ${result.copied??targetCells.length} celda${targetCells.length===1?"":"s"}`);
    await loadData();
  }

  async function fillShopperCells(drag:FillDrag){
    ...
    const sourceWorkDate=dateKeys[drag.sourceDay];
    const sourceTurn=shopperTurns.find(turn=>turn.staff_id===source?.id&&turn.work_date===sourceWorkDate);
    const targetCells=fillTargets(drag).map(target=>({staff:visible[target.row],day:target.day,workDate:dateKeys[target.day]})).filter(target=>Boolean(target.staff));
    if(!source||!sourceTurn?.turn_code||!sourceTurn.shift_type_id||!targetCells.length)return;
    const turnCode=sourceTurn.turn_code,shiftTypeId=sourceTurn.shift_type_id;
    const overwriteCount=targetCells.filter(target=>shopperTurns.some(turn=>turn.staff_id===target.staff.id&&turn.work_date===target.workDate)).length;
    if(overwriteCount&&!window.confirm(`Se reemplazarán ${overwriteCount} turno${overwriteCount===1?"":"s"} existente${overwriteCount===1?"":"s"}. ¿Continuar?`))return;
    const mutationVersion=beginShopperMutation();
    if(mutationVersion===null)return;
    const targetKeys=new Set(targetCells.map(target=>`${target.staff.id}-${target.workDate}`));
    setShopperTurns(current=>{
      const next=current.map(turn=>targetKeys.has(`${turn.staff_id}-${turn.work_date}`)?{...turn,turn_code:turnCode,shift_type_id:shiftTypeId}:turn);
      const existing=new Set(next.map(turn=>`${turn.staff_id}-${turn.work_date}`));
      targetCells.forEach((target,index)=>{const key=`${target.staff.id}-${target.workDate}`;if(!existing.has(key))next.push({id:-(Date.now()+index),staff_id:target.staff.id,work_date:target.workDate,turn_code:turnCode,shift_type_id:shiftTypeId});});
      return next;
    });
    setNotice(`Copiando turno en ${targetCells.length} celda${targetCells.length===1?"":"s"}…`);
    try{
      const response=await apiFetch("/api/shoppers",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({
        action:"fillTurns",cells:targetCells.map(target=>({staffId:target.staff.id,workDate:target.workDate})),shiftTypeId
      })});
      const result=await response.json().catch(()=>({error:"No se pudo copiar el turno"})) as {copied?:number;error?:string};
      ...
      if(!response.ok){
        ...
        setNotice(`✓ Turno ${turnCode} copiado en ${result.copied??targetCells.length} celda${targetCells.length===1?"":"s"}`);
      }
    }finally{
      endShopperMutation();
    }
  ...
  async function endFillDrag(event:ReactPointerEvent<HTMLButtonElement>){
    ...
    if(current.targetIndex===current.sourceIndex&&current.targetDay===current.sourceDay){
      setNotice("Mantén presionado el cuadro naranja y arrástralo en cualquier dirección para copiar el turno");
      return;
    }
    if(current.kind==="supervisor")await fillSupervisorCells(current);
    else await fillShopperCells(current);
  }
  ...
  async function createShopperShift(form:FormData){
    const selected=editShopperShift?.is_general?null:data?.locations.find(l=>l.id===Number(form.get("locationId")));
    if(!editShopperShift?.is_general&&!selected){setNotice("Selecciona un local");return;}
    const response=await apiFetch("/api/shoppers",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({
      action:editShopperShift?"updateShiftType":"addShiftType",id:editShopperShift?.id,code:form.get("code"),label:form.get("label"),start:form.get("start"),end:form.get("end"),
    ...
    const result=await response.json().catch(()=>({error:"No se pudo guardar el turno"}));
    if(!response.ok){setNotice(`Error: ${result.error}`);return;}
    setAddShopperShift(false);setEditShopperShift(null);setNotice(editShopperShift||result.updated?"✓ Turno actualizado":"✓ Turno privado creado para tu cuenta");await loadShoppers();
  }

  async function confirmDeleteShopperShift(){
    if(!deleteShopperShift||deletingShopperShift)return;
    setDeletingShopperShift(true);
    const response=await apiFetch("/api/shoppers",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"deleteShiftType",id:deleteShopperShift.id})});
    const result=await response.json().catch(()=>({error:"No se pudo eliminar el turno"}));
    setDeletingShopperShift(false);
    if(!response.ok){setNotice(`Error: ${result.error}`);return;}
    setDeleteShopperShift(null);setNotice("✓ Turno eliminado de las opciones nuevas");await loadShoppers();
  }
  ...
  function shopperShiftSchedule(shiftType:ShopperShiftType|undefined,code:string){
    if(!shiftType)return code;
    if(shiftType.is_free)return shiftType.label||shopperShiftLabel(shiftType);
    if(shiftType.start_time&&shiftType.end_time)return `${shiftType.start_time.slice(0,5)}–${shiftType.end_time.slice(0,5)}`;
    return code;
  }
  ...
  return (
    <main className={`app-shell ${sidebarCollapsed?"sidebar-collapsed":""}`}>
      ...
      <section className="workspace">
        ...
        {active==="Shoppers"&&<section className="schedule-card shopper-schedule" ref={shopperScheduleRef}>
          ...
          {(()=>{
            ...
            const types=visible.flatMap(s=>dateKeys.map(d=>{const turn=shopperTurns.find(t=>t.staff_id===s.id&&t.work_date===d);return shopperShiftFor(turn?.turn_code||"",s.location_id,turn?.shift_type_id)}).filter(Boolean) as ShopperShiftType[]);
            const daily=dateKeys.map((date,index)=>{
              const dayTypes=visible.map(s=>{const turn=shopperTurns.find(t=>t.staff_id===s.id&&t.work_date===date);return shopperShiftFor(turn?.turn_code||"",s.location_id,turn?.shift_type_id)}).filter(Boolean) as ShopperShiftType[];
            ...
            <div className="shopper-table-view-bar"><span>{showShopperHours?"Mostrando las horas de cada turno":"Mostrando los códigos de turno"}</span><button type="button" className={`view-hours-action${showShopperHours?" active":""}`} onClick={()=>setShowShopperHours(value=>!value)}>{showShopperHours?"Ocultar horario":"◷ Ver horario"}</button></div>
            <div className={`table-wrap shopper-scalable${visible.length>20?" shopper-table-compact":""}`}><table><thead><tr><th>{shopperCategory==="purchase"?"Asesor de compra":"Repartidor"}</th>{days.map(d=><th key={d}>{d}</th>)}<th>Aperturas</th><th>Cierres</th>{isAdmin&&<th>Horas</th>}</tr></thead><tbody>{visible.map((staff,rowIndex)=>{const weeklyTurns=dateKeys.map(d=>shopperTurns.find(t=>t.staff_id===staff.id&&t.work_date===d));const weekly=weeklyTurns.map(turn=>turn?.turn_code||"");const weeklyTypes=weeklyTurns.map(turn=>shopperShiftFor(turn?.turn_code||"",staff.location_id,turn?.shift_type_id));const weeklyHours=weeklyTypes.reduce((total,type)=>total+shopperShiftHours(type),0);return <tr key={staff.id}><td><div className="person"><span className="avatar">{staff.name.split(" ").map(x=>x[0]).join("").slice(0,2)}</span><div className="person-info"><div className="shopper-name-line"><strong className="shopper-name-text">{staff.name}</strong><div className="shopper-row-actions"><button className="shopper-action edit" onClick={()=>setEditShopper(staff)} title={`Editar a ${staff.name}`} aria-label={`Editar a ${staff.name}`}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20h4l11-11-4-4L4 16v4Zm12.2-16.2 4 4 1.1-1.1a1.4 1.4 0 0 0 0-2l-2-2a1.4 1.4 0 0 0-2 0l-1.1 1.1Z"/></svg></button><button className="shopper-action delete" onClick={()=>setDeleteShopper(staff)} title={`Eliminar a ${staff.name}`} aria-label={`Eliminar a ${staff.name}`}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 3h8l1 2h4v2H3V5h4l1-2Zm-2 6h12l-1 12H7L6 9Zm4 2v7h2v-7h-2Zm4 0v7h2v-7h-2Z"/></svg></button></div></div><small>{staff.employment_type} · {staff.location_name} <span className="shopper-inline-id">ID {staff.shopper_external_id||"—"}</span></small></div></div></td>{dateKeys.map((d,i)=><td key={d} data-fill-kind="shopper" data-fill-row={rowIndex} data-fill-day={i}><div className={`fillable-cell${fillCellClass("shopper",rowIndex,i)}`}><button className={`turn-code turn-${weekly[i]||"empty"}${showShopperHours&&weekly[i]?" hours-view":""}`} onClick={()=>openShopperTurnModal(staff,d)}>{weekly[i]?(showShopperHours?shopperShiftSchedule(weeklyTypes[i],weekly[i]):weekly[i]):"＋"}</button>{weekly[i]&&<button type="button" className="fill-handle" aria-label={`Copiar el turno de ${staff.name} en varias celdas`} title="Arrastra en cualquier dirección para copiar" onClick={event=>event.stopPropagation()} onPointerDown={event=>beginFillDrag("shopper",rowIndex,i,event)} onPointerMove={moveFillDrag} onPointerUp={event=>void endFillDrag(event)} onPointerCancel={cancelFillDrag} />}</div></td>)}<td className="hours">{weeklyTypes.filter(t=>t?.counts_opening).length}</td><td className="hours">{weeklyTypes.filter(t=>t?.counts_closing).length}</td>{isAdmin&&<td className="hours shopper-hours">{displayHours(weeklyHours)} h</td>}</tr>})}</tbody></table></div></>
      ...
      {(addShopperShift||editShopperShift)&&<div className="modal-backdrop" onMouseDown={()=>{setAddShopperShift(false);setEditShopperShift(null)}}><form className="modal shift-type-editor" onSubmit={e=>{e.preventDefault();void createShopperShift(new FormData(e.currentTarget));}} onMouseDown={e=>e.stopPropagation()}>
        ...
        <span className="modal-kicker">{editShopperShift?"EDITAR TURNO":"NUEVO TURNO"}</span>
        <h2>{editShopperShift?`Editar ${editShopperShift.code}`:"Crear turno de shoppers"}</h2>
        <p>{editShopperShift?.is_general?"Este turno es general; los cambios se aplicarán para todos.":"Puedes cambiar el código, nombre, horario, categoría y clasificación."}</p>
        {editShopperShift?.is_general?<div className="general-shift-notice">Turno general disponible para todos los locales</div>:<label>Local<select name="locationId" required defaultValue={editShopperShift?.location_id??data?.locations.find(l=>l.name===location)?.id??""}><option value="" disabled>Selecciona el local</option>{data?.locations.map(l=><option key={l.id} value={l.id}>{l.name} · {l.city}</option>)}</select></label>}
        <div className="time-row"><label>Código<input name="code" required maxLength={6} placeholder="A" defaultValue={editShopperShift?.code??""} /></label><label>Nombre<input name="label" required placeholder="Apertura" defaultValue={editShopperShift?.label??""} /></label></div>
        <div className="time-row"><label>Hora de inicio<input name="start" type="time" defaultValue={editShopperShift?.start_time?.slice(0,5)??"06:00"} /></label><label>Hora de fin<input name="end" type="time" defaultValue={editShopperShift?.end_time?.slice(0,5)??"14:00"} /></label></div>
        <label>Disponible para<select name="category" defaultValue={editShopperShift?.category??shopperCategory}><option value="purchase">Asesores de compra</option><option value="delivery">Repartidores</option><option value="both">Ambos</option></select></label>
        <fieldset className="shift-flags"><legend>Clasificación</legend><label><input type="checkbox" name="countsOpening" defaultChecked={editShopperShift?.counts_opening??false} /> Cuenta como apertura</label><label><input type="checkbox" name="countsClosing" defaultChecked={editShopperShift?.counts_closing??false} /> Cuenta como cierre</label><label><input type="checkbox" name="isFree" defaultChecked={editShopperShift?.is_free??false} /> Es libre o vacaciones</label></fieldset>
        <button className="primary save">{editShopperShift?"Guardar cambios":"Crear turno"}</button>
      </form></div>}
      {deleteShopperShift&&<div className="modal-backdrop" onMouseDown={()=>!deletingShopperShift&&setDeleteShopperShift(null)}><div className="modal confirm-delete-modal" role="alertdialog" aria-modal="true" aria-labelledby="delete-shopper-shift-title" onMouseDown={e=>e.stopPropagation()}><button type="button" className="close" disabled={deletingShopperShift} onClick={()=>setDeleteShopperShift(null)}>×</button><span className="delete-warning-icon">!</span><span className="modal-kicker">ELIMINAR TURNO</span><h2 id="delete-shopper-shift-title">¿Eliminar el turno {deleteShopperShift.code}?</h2><p>Dejará de aparecer para nuevas asignaciones. Los horarios donde ya fue utilizado conservarán su información histórica.</p><div className="confirm-actions"><button type="button" className="secondary" disabled={deletingShopperShift} onClick={()=>setDeleteShopperShift(null)}>Cancelar</button><button type="button" className="danger-button" disabled={deletingShopperShift} onClick={()=>void confirmDeleteShopperShift()}>{deletingShopperShift?"Eliminando…":"Sí, eliminar turno"}</button></div></div></div>}
      {shopperModal&&(()=>{
        const available=shopperShiftTypes.filter(type=>type.active&&(type.location_id===null||type.location_id===shopperModal.staff.location_id)&&(type.category==="both"||type.category===shopperModal.staff.category));
        ...
        return <div className="modal-backdrop" onMouseDown={()=>setShopperModal(null)}><div className="modal turn-modal shopper-turn-combo" onMouseDown={e=>e.stopPropagation()}><button type="button" className="close" onClick={()=>setShopperModal(null)}>×</button><span className="modal-kicker">ASIGNAR TURNO</span><h2>{shopperModal.staff.name}</h2><p>{shopperModal.date} · {shopperModal.staff.location_name}</p><label className="turn-search-label">Escribe o selecciona el turno<div className="turn-search-input"><input autoFocus value={shopperTurnInput} onChange={e=>setShopperTurnInput(e.target.value.toUpperCase())} onKeyDown={e=>{if(e.key!=="Enter")return;e.preventDefault();const code=shopperTurnInput.trim().toUpperCase();const selected=available.find(type=>type.code.toUpperCase()===code&&!type.is_general)??available.find(type=>type.code.toUpperCase()===code);if(selected)void saveShopperTurn(selected.id);else setNotice("Turno no registrado para este local");}} placeholder="Escribe A, T, L…" maxLength={6}/><kbd>↵</kbd></div></label><small className="turn-search-help">Puedes repetir códigos: elige General o Mi turno.</small><div className="turn-options">{matches.map(type=><article className="turn-option-card" key={`${type.id}-${type.code}`}><button type="button" className="turn-option-select" onClick={()=>void saveShopperTurn(type.id)}><strong>{type.code}</strong><span>{type.start_time&&type.end_time?`${type.start_time.slice(0,5)}–${type.end_time.slice(0,5)} · `:""}{shopperShiftLabel(type)} · {type.is_general?"General":"Mi turno"}</span></button><div className="turn-option-actions"><button type="button" onClick={()=>{setShopperModal(null);setEditShopperShift(type)}}>Editar</button><button type="button" className="remove" onClick={()=>{setShopperModal(null);setDeleteShopperShift(type)}}>Eliminar</button></div></article>)}</div>{matches.length===0&&<div className="turn-search-empty"><strong>Turno no registrado</strong><span>Prueba con otro código disponible para este local.</span></div>}</div></div>;
      })()}
